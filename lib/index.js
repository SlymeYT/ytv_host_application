...
var startProxy = function(port, proxyUrl, proxyPartial, credentials, origin) {
  ...
  //proxy.setHeader('User-Agent', 'Mozilla/5.0 (SmartHub; SMART-TV; U; Linux/SmartTV) AppleWebKit/531.2+ (KHTML, Like Gecko) WebBrowser/1.0 SmartTV Safari/531.2+');
  proxy.use('/' + cleanProxyPartial, function(req, res) {
	  //res.header('User-Agent', 'Mozilla/5.0 (SmartHub; SMART-TV; U; Linux/SmartTV) AppleWebKit/531.2+ (KHTML, Like Gecko) WebBrowser/1.0 SmartTV Safari/531.2+');
    try {
	...
	res.setHeader('User-Agent', 'Mozilla/5.0 (SmartHub; SMART-TV; U; Linux/SmartTV) AppleWebKit/531.2+ (KHTML, Like Gecko) WebBrowser/1.0 SmartTV Safari/531.2+');
  });
  ...
  console.log(
    chalk.cyan(
      'Server hostname: AdamZRX YOUTUBE SERVER' 
    )
  );
};
